...
router.get('/collections/:name', (req, res) => {
  ...
  res.json({ items: db.list(req.params.name) });
});

router.post('/collections/:name', (req, res) => {
  if (!MANAGED_COLLECTIONS.includes(req.params.name)) return res.status(404).json({ error: 'Unknown collection.' });
  const item = db.create(req.params.name, { ...req.body, status: req.body.status || 'draft', author: req.user.name });
...
router.put('/collections/:name/:id', (req, res) => {
  if (!MANAGED_COLLECTIONS.includes(req.params.name)) return res.status(404).json({ error: 'Unknown collection.' });
  const item = db.update(req.params.name, req.params.id, req.body);
...
router.delete('/collections/:name/:id', (req, res) => {
  if (!MANAGED_COLLECTIONS.includes(req.params.name)) return res.status(404).json({ error: 'Unknown collection.' });
  const ok = db.remove(req.params.name, req.params.id);
...
router.get('/users', (_req, res) => {
  const users = db.list('users').map(({ passwordHash, ...u }) => u);
  res.json({ users });
});
...
router.get('/analytics', (_req, res) => {
  ...
  res.json({
    ...
    courseEnrollments: db.list('progress').length,
    liveClassesScheduled: db.list('liveClasses').length,
    languageCourseCount: db.list('languageCourses').length,
  });
});
